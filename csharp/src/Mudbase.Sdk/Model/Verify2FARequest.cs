...
namespace Mudbase.Sdk.Model
{
    ...
    public partial class Verify2FARequest : IValidatableObject
    {
        ...
        [JsonConstructor]
        public Verify2FARequest(string token)
        {
            Token = token;
            OnCreated();
        }
    ...
    public class Verify2FARequestJsonConverter : JsonConverter<Verify2FARequest>
    {
        ...
        public override Verify2FARequest Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (!token.IsSet)
                throw new ArgumentException("Property is required for class Verify2FARequest.", nameof(token));

            if (token.IsSet && token.Value == null)
                throw new ArgumentNullException(nameof(token), "Property is not nullable for class Verify2FARequest.");

            return new Verify2FARequest(token.Value!);
        }
        ...
        {
            if (verify2FARequest.Token == null)
                throw new ArgumentNullException(nameof(verify2FARequest.Token), "Property is required for class Verify2FARequest.");

            writer.WriteString("token", verify2FARequest.Token);
        }
    }
}
