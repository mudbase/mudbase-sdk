...
class ConfigureOAuthProviderRequest(BaseModel):
    ...
    """ # noqa: E501
    enabled: StrictBool = Field(description="Whether the OAuth provider is enabled", json_schema_extra={"examples": [True]})
    client_id: StrictStr = Field(description="OAuth client ID from the provider", alias="clientId", json_schema_extra={"examples": ["123456789-abcdefghijklmnop.apps.googleusercontent.com"]})
    client_secret: StrictStr = Field(description="OAuth client secret from the provider", alias="clientSecret", json_schema_extra={"examples": ["GOCSPX-abcdefghijklmnopqrstuvwxyz"]})
