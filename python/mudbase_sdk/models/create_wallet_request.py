...
class CreateWalletRequest(BaseModel):
    ...
    CreateWalletRequest
    """ # noqa: E501
    currency: StrictStr = Field(description="Currency. USDT requires network (ETH, BSC, TRX, SOL, POLYGON). All platform chains supported for testing non-custodial flows.")
    project_id: Optional[StrictStr] = Field(default=None, description="Optional project ID", alias="projectId")
    network: Optional[StrictStr] = Field(default=None, description="Required for USDT; one of ETH, BSC, TRX, SOL, POLYGON")
    private_key: Optional[StrictStr] = Field(default=None, description="Optional custom private key", alias="privateKey")
    label: Optional[StrictStr] = None
    __properties: ClassVar[List[str]] = ["currency", "projectId", "network", "privateKey", "label"]

    @field_validator('currency')
    def currency_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['BTC', 'LTC', 'DOGE', 'ETH', 'CELO', 'SOL', 'TRX', 'TON', 'MATIC', 'BNB', 'AVAX', 'ADA', 'USDT']):
            raise ValueError("must be one of enum values ('BTC', 'LTC', 'DOGE', 'ETH', 'CELO', 'SOL', 'TRX', 'TON', 'MATIC', 'BNB', 'AVAX', 'ADA', 'USDT')")
        return value
