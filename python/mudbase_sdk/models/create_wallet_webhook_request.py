...
class CreateWalletWebhookRequest(BaseModel):
    ...
    def events_validate_enum(cls, value):
        """Validates the enum"""
        for i in value:
            if i not in set(['wallet.balance.updated', 'wallet.transaction.confirmed', 'wallet.transaction.failed', 'wallet.transaction.detected', 'wallet.transaction.broadcast', 'wallet.token.balance.updated', 'wallet.address.created', 'wallet.address.deactivated']):
                raise ValueError("each list item must be one of ('wallet.balance.updated', 'wallet.transaction.confirmed', 'wallet.transaction.failed', 'wallet.transaction.detected', 'wallet.transaction.broadcast', 'wallet.token.balance.updated', 'wallet.address.created', 'wallet.address.deactivated')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "filters": CreateWalletWebhookRequestFilters.from_dict(obj["filters"]) if obj.get("filters") is not None else None,
