...
class EstimateNetworkFeeRequest(BaseModel):
    """
    EstimateNetworkFeeRequest
    """ # noqa: E501
    currency: StrictStr = Field(description="Currency code")
    amount: Union[Annotated[float, Field(strict=True, gt=0)], Annotated[int, Field(strict=True, gt=0)]] = Field(description="Transaction amount (used for display; fee is chain-based)")
    network: Optional[StrictStr] = Field(default=None, description="Required for USDT; network on which USDT is sent")
    __properties: ClassVar[List[str]] = ["currency", "amount", "network"]

    @field_validator('currency')
    def currency_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['BTC', 'ETH', 'BNB', 'LTC', 'SOL', 'TRX', 'USDT', 'MATIC', 'AVAX', 'CELO', 'DOGE', 'TON', 'ADA']):
            raise ValueError("must be one of enum values ('BTC', 'ETH', 'BNB', 'LTC', 'SOL', 'TRX', 'USDT', 'MATIC', 'AVAX', 'CELO', 'DOGE', 'TON', 'ADA')")
        return value
    ...
    def network_validate_enum(cls, value):
        ...
        if value not in set(['ETH', 'BSC', 'TRX', 'SOL', 'POLYGON']):
            raise ValueError("must be one of enum values ('ETH', 'BSC', 'TRX', 'SOL', 'POLYGON')")
        return value
