...
class EstimateNonCustodialGasRequest(BaseModel):
    """
    EstimateNonCustodialGasRequest
    """ # noqa: E501
    chain: StrictStr = Field(description="Chain id. For EVM, transaction is required. For non-EVM (UTXO, Solana, Tron, TON, Cardano) only chain is needed.")
    transaction: Optional[EstimateNonCustodialGasRequestTransaction] = None
    __properties: ClassVar[List[str]] = ["chain", "transaction"]

    @field_validator('chain')
    def chain_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ethereum', 'binance', 'bsc', 'polygon', 'arbitrum', 'optimism', 'base', 'avalanche', 'celo', 'bitcoin', 'litecoin', 'dogecoin', 'solana', 'tron', 'ton', 'cardano']):
            raise ValueError("must be one of enum values ('ethereum', 'binance', 'bsc', 'polygon', 'arbitrum', 'optimism', 'base', 'avalanche', 'celo', 'bitcoin', 'litecoin', 'dogecoin', 'solana', 'tron', 'ton', 'cardano')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "chain": obj.get("chain"),
            "transaction": EstimateNonCustodialGasRequestTransaction.from_dict(obj["transaction"]) if obj.get("transaction") is not None else None
        })
        return _obj
