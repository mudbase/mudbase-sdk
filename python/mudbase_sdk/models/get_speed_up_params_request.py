...
class GetSpeedUpParamsRequest(BaseModel):
    """
    GetSpeedUpParamsRequest
    """ # noqa: E501
    tx_id: Optional[StrictStr] = Field(default=None, description="WalletTransaction _id (MongoDB ObjectId)", alias="txId")
    tx_hash: Optional[StrictStr] = Field(default=None, description="mainTxHash or txHash of the stuck transaction", alias="txHash")
    chain: StrictStr = Field(description="EVM chain (speed-up is EVM only)")
    __properties: ClassVar[List[str]] = ["txId", "txHash", "chain"]

    @field_validator('chain')
    def chain_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ethereum', 'polygon', 'arbitrum', 'optimism', 'base', 'bsc', 'binance', 'avalanche', 'celo']):
            raise ValueError("must be one of enum values ('ethereum', 'polygon', 'arbitrum', 'optimism', 'base', 'bsc', 'binance', 'avalanche', 'celo')")
        return value
