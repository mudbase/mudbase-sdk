...
class SimulateAppPermissionsRequest(BaseModel):
    ...
    """ # noqa: E501
    role: StrictStr = Field(description="App role slug (same as `roleSlug` elsewhere)", json_schema_extra={"examples": ["customer"]})
    role_slug: Optional[StrictStr] = Field(default=None, description="Alias for `role`", alias="roleSlug")
    operation_id: Optional[StrictStr] = Field(default=None, description="OpenAPI operationId (e.g. `sendEmail`, `executeIntegration`). When set, path simulation is optional.", alias="operationId", json_schema_extra={"examples": ["sendEmail"]})
    method: Optional[StrictStr] = Field(default=None, json_schema_extra={"examples": ["POST"]})
    pathname: Optional[StrictStr] = Field(default=None, description="Full path e.g. `/api/messaging/projects/{id}/messaging/email`", json_schema_extra={"examples": ["/api/messaging/projects/685ad30be129932fbb7a1047/messaging/email"]})
