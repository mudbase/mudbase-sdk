...
module MudbaseSDK
  class AdminMemberRolePatchRequest < ApiModelBase
    attr_accessor :role
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::AdminMemberRolePatchRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::AdminMemberRolePatchRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      role_validator = EnumAttributeValidator.new('String', ["owner", "admin", "developer", "viewer"])
    ...
    def role=(role)
      validator = EnumAttributeValidator.new('String', ["owner", "admin", "developer", "viewer"])
