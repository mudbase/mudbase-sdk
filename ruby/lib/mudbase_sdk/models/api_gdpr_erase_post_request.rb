...
module MudbaseSDK
  class ApiGdprErasePostRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::ApiGdprErasePostRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::ApiGdprErasePostRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      confirm_validator = EnumAttributeValidator.new('String', ["DELETE"])
      return false unless confirm_validator.valid?(@confirm)
      true
    end
    ...
    def confirm=(confirm)
      ...
      unless validator.valid?(confirm)
        fail ArgumentError, "invalid value for \"confirm\", must be one of #{validator.allowable_values}."
      end
      @confirm = confirm
    end
    ...
    def hash
      [confirm, current_password, totp_token].hash
    end
