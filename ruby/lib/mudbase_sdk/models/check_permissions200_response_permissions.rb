...
module MudbaseSDK
  class CheckPermissions200ResponsePermissions < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::CheckPermissions200ResponsePermissions` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::CheckPermissions200ResponsePermissions`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'system')
        if (value = attributes[:'system']).is_a?(Array)
          self.system = value
        end
      end
      ...
      if attributes.key?(:'combined')
        if (value = attributes[:'combined']).is_a?(Array)
          self.combined = value
        end
      end
    end
