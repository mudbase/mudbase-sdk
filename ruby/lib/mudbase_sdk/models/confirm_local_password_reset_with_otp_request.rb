...
module MudbaseSDK
  class ConfirmLocalPasswordResetWithOtpRequest < ApiModelBase
    attr_accessor :email
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::ConfirmLocalPasswordResetWithOtpRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::ConfirmLocalPasswordResetWithOtpRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def otp=(otp)
      ...
      if otp.to_s.length < 6
        fail ArgumentError, 'invalid value for "otp", the character length must be greater than or equal to 6.'
      end
