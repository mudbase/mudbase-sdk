  ...
  class CreateApiKeyRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::CreateApiKeyRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::CreateApiKeyRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      pattern = Regexp.new(/^[0-9a-fA-F]{24}$/)
      if @project_id !~ pattern
        invalid_properties.push("invalid value for \"project_id\", must conform to the pattern #{pattern}.")
      end
    ...
    def valid?
      ...
      return false if @project_id !~ Regexp.new(/^[0-9a-fA-F]{24}$/)
      true
    end
    ...
    def project_id=(project_id)
      ...
      pattern = Regexp.new(/^[0-9a-fA-F]{24}$/)
      if project_id !~ pattern
        fail ArgumentError, "invalid value for \"project_id\", must conform to the pattern #{pattern}."
      end
    ...
    def hash
      [name, project_id, permissions, rate_limit, expires_at].hash
    end
