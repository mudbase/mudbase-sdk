...
module MudbaseSDK
  class CreateOrUpdateFeeSettingsRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::CreateOrUpdateFeeSettingsRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::CreateOrUpdateFeeSettingsRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'payout_address')
        self.payout_address = attributes[:'payout_address']
      end
    ...
    def list_invalid_properties
      ...
      if !@fee_amount.nil? && @fee_amount < 0
        invalid_properties.push('invalid value for "fee_amount", must be greater than or equal to 0.')
      end

      if !@payout_threshold.nil? && @payout_threshold < 0
        invalid_properties.push('invalid value for "payout_threshold", must be greater than or equal to 0.')
      end
    ...
    def valid?
      ...
      currency_validator = EnumAttributeValidator.new('String', ["BTC", "ETH", "BNB", "LTC", "SOL", "TRX", "USDT-ETH", "USDT-BSC", "USDT-TRX", "USDT-SOL"])
      return false unless currency_validator.valid?(@currency)
      return false if !@fee_amount.nil? && @fee_amount < 0
      return false if !@payout_threshold.nil? && @payout_threshold < 0
      true
    end
    ...
    def currency=(currency)
      validator = EnumAttributeValidator.new('String', ["BTC", "ETH", "BNB", "LTC", "SOL", "TRX", "USDT-ETH", "USDT-BSC", "USDT-TRX", "USDT-SOL"])
    ...
    def fee_amount=(fee_amount)
      if fee_amount.nil?
        fail ArgumentError, 'fee_amount cannot be nil'
      end

      if fee_amount < 0
        fail ArgumentError, 'invalid value for "fee_amount", must be greater than or equal to 0.'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] payout_threshold Value to be assigned
    def payout_threshold=(payout_threshold)
      if payout_threshold.nil?
        fail ArgumentError, 'payout_threshold cannot be nil'
      end

      if payout_threshold < 0
        fail ArgumentError, 'invalid value for "payout_threshold", must be greater than or equal to 0.'
      end
    ...
    def hash
      [currency, enabled, fee_amount, payout_address, payout_threshold].hash
    end
