...
module MudbaseSDK
  ...
  class CreatePlanRequestLimits < ApiModelBase
    ...
    def self.openapi_types
      {
        ...
        :'bandwidth' => :'Float',
        :'users' => :'Float',
        :'custom_limits' => :'Array<CreatePlanRequestLimitsCustomLimitsInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::CreatePlanRequestLimits` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::CreatePlanRequestLimits`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'custom_limits')
        if (value = attributes[:'custom_limits']).is_a?(Array)
    ...
    def hash
      [api_calls, storage, bandwidth, users, custom_limits].hash
    end
