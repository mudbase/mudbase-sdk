...
module MudbaseSDK
  class CreateRoleRequest < ApiModelBase
    attr_accessor :name
    ...
    def self.openapi_types
      {
        :'name' => :'String',
        :'description' => :'String',
        :'permissions' => :'Array<CreateRoleRequestPermissionsInner>',
        :'hierarchy' => :'Float',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::CreateRoleRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::CreateRoleRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@hierarchy.nil? && @hierarchy > 100
        invalid_properties.push('invalid value for "hierarchy", must be smaller than or equal to 100.')
      end
    ...
    def valid?
      ...
      return false if !@hierarchy.nil? && @hierarchy > 100
    ...
    # Custom attribute writer method with validation
    # @param [Object] hierarchy Value to be assigned
    def hierarchy=(hierarchy)
      if hierarchy.nil?
        fail ArgumentError, 'hierarchy cannot be nil'
      end

      if hierarchy > 100
        fail ArgumentError, 'invalid value for "hierarchy", must be smaller than or equal to 100.'
      end

      if hierarchy < 0
        fail ArgumentError, 'invalid value for "hierarchy", must be greater than or equal to 0.'
      end
    ...
    def hash
      [name, description, permissions, hierarchy, collection_permissions].hash
    end
