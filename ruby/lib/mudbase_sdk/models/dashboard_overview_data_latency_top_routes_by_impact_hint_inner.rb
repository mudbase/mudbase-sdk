...
module MudbaseSDK
  class DashboardOverviewDataLatencyTopRoutesByImpactHintInner < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::DashboardOverviewDataLatencyTopRoutesByImpactHintInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::DashboardOverviewDataLatencyTopRoutesByImpactHintInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'route_key')
        self.route_key = attributes[:'route_key']
      end
    ...
    def hash
      [route_key, p50_ms, p95_ms, count, impact_score].hash
    end
