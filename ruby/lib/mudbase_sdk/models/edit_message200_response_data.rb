...
module MudbaseSDK
  class EditMessage200ResponseData < ApiModelBase
    attr_accessor :_id

    attr_accessor :content

    attr_accessor :is_e2ee
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::EditMessage200ResponseData` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::EditMessage200ResponseData`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'is_e2ee')
        self.is_e2ee = attributes[:'is_e2ee']
      end
    ...
    def hash
      [_id, content, is_e2ee, e2ee, edited_at].hash
    end
