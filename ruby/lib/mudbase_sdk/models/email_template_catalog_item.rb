...
module MudbaseSDK
  ...
  class EmailTemplateCatalogItem < ApiModelBase
    attr_accessor :name

    # True if this project has a stored override for this template name.
    attr_accessor :is_customized

    # Which layer is used at send time for this name.
    attr_accessor :effective_source

    attr_accessor :subject_snippet
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::EmailTemplateCatalogItem` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::EmailTemplateCatalogItem`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'is_customized')
        self.is_customized = attributes[:'is_customized']
      end

      if attributes.key?(:'effective_source')
        self.effective_source = attributes[:'effective_source']
      end

      if attributes.key?(:'subject_snippet')
        self.subject_snippet = attributes[:'subject_snippet']
      end
    ...
    def valid?
      ...
      effective_source_validator = EnumAttributeValidator.new('String', ["project", "global", "builtin"])
      return false unless effective_source_validator.valid?(@effective_source)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] effective_source Object to be assigned
    def effective_source=(effective_source)
      validator = EnumAttributeValidator.new('String', ["project", "global", "builtin"])
      unless validator.valid?(effective_source)
        fail ArgumentError, "invalid value for \"effective_source\", must be one of #{validator.allowable_values}."
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          subject_snippet == o.subject_snippet &&
    ...
    def hash
      [name, is_customized, effective_source, subject_snippet, updated_at, version].hash
    end
