...
module MudbaseSDK
  class EraseUserData409Response < ApiModelBase
    attr_accessor :error

    attr_accessor :sole_owned_orgs
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::EraseUserData409Response` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::EraseUserData409Response`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'sole_owned_orgs')
        if (value = attributes[:'sole_owned_orgs']).is_a?(Array)
          self.sole_owned_orgs = value
        end
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          error == o.error &&
          sole_owned_orgs == o.sole_owned_orgs
    end
