...
module MudbaseSDK
  class GetAdminPayoutDashboard200ResponseData < ApiModelBase
    ...
    def self.openapi_types
      {
        ...
        :'by_currency' => :'Array<GetAdminPayoutDashboard200ResponseDataByCurrencyInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::GetAdminPayoutDashboard200ResponseData` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::GetAdminPayoutDashboard200ResponseData`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'total_pending')
        self.total_pending = attributes[:'total_pending']
      end
      ...
      if attributes.key?(:'by_currency')
        if (value = attributes[:'by_currency']).is_a?(Array)
          self.by_currency = value
        end
      end
    end
    ...
    def hash
      [total_pending, total_completed, total_failed, by_currency].hash
    end
