...
module MudbaseSDK
  class GetAvailableOAuthProviders200ResponseProvidersInner < ApiModelBase
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::GetAvailableOAuthProviders200ResponseProvidersInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::GetAvailableOAuthProviders200ResponseProvidersInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'default_scope')
        if (value = attributes[:'default_scope']).is_a?(Array)
    ...
    def hash
      [name, display_name, strategy, default_scope, callback_url, required_fields].hash
    end
