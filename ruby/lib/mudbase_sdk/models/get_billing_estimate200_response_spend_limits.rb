...
module MudbaseSDK
  class GetBillingEstimate200ResponseSpendLimits < ApiModelBase
    attr_accessor :soft_limit_cents

    attr_accessor :hard_limit_cents

    attr_accessor :spend_blocked
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::GetBillingEstimate200ResponseSpendLimits` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::GetBillingEstimate200ResponseSpendLimits`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'soft_limit_cents')
        self.soft_limit_cents = attributes[:'soft_limit_cents']
      end

      if attributes.key?(:'hard_limit_cents')
        self.hard_limit_cents = attributes[:'hard_limit_cents']
      end

      if attributes.key?(:'spend_blocked')
        self.spend_blocked = attributes[:'spend_blocked']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          soft_limit_cents == o.soft_limit_cents &&
          hard_limit_cents == o.hard_limit_cents &&
          spend_blocked == o.spend_blocked
    end
    ...
    def hash
      [soft_limit_cents, hard_limit_cents, spend_blocked].hash
    end
