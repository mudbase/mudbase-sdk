...
module MudbaseSDK
  class GetComplianceSummary200ResponseComplianceGdpr < ApiModelBase
    attr_accessor :data_export_enabled

    attr_accessor :data_erasure_enabled

    attr_accessor :consent_management
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::GetComplianceSummary200ResponseComplianceGdpr` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::GetComplianceSummary200ResponseComplianceGdpr`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'data_export_enabled')
        self.data_export_enabled = attributes[:'data_export_enabled']
      end

      if attributes.key?(:'data_erasure_enabled')
        self.data_erasure_enabled = attributes[:'data_erasure_enabled']
      end

      if attributes.key?(:'consent_management')
        self.consent_management = attributes[:'consent_management']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          data_export_enabled == o.data_export_enabled &&
          data_erasure_enabled == o.data_erasure_enabled &&
          consent_management == o.consent_management
    end
    ...
    def hash
      [data_export_enabled, data_erasure_enabled, consent_management].hash
    end
