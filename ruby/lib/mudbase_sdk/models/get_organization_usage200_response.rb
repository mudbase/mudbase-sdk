...
module MudbaseSDK
  class GetOrganizationUsage200Response < ApiModelBase
    attr_accessor :usage
    ...
    def self.openapi_types
      {
        ...
        :'plan' => :'Plan',
        :'billing' => :'Billing',
        :'suborgs' => :'Array<GetOrganizationUsage200ResponseAllOfSuborgsInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::GetOrganizationUsage200Response` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::GetOrganizationUsage200Response`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'suborgs')
        if (value = attributes[:'suborgs']).is_a?(Array)
          self.suborgs = value
        end
      end
    end
    ...
    def hash
      [usage, limits, plan, billing, suborgs].hash
    end
