...
module MudbaseSDK
  class GetProjectCaptchaConfig200ResponseCaptcha < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::GetProjectCaptchaConfig200ResponseCaptcha` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::GetProjectCaptchaConfig200ResponseCaptcha`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@min_score.nil? && @min_score > 1
        invalid_properties.push('invalid value for "min_score", must be smaller than or equal to 1.')
      end

      if !@min_score.nil? && @min_score < 0
        invalid_properties.push('invalid value for "min_score", must be greater than or equal to 0.')
      end
    ...
    def valid?
      ...
      version_validator = EnumAttributeValidator.new('String', ["v2", "v3"])
      return false unless version_validator.valid?(@version)
      return false if !@min_score.nil? && @min_score > 1
      return false if !@min_score.nil? && @min_score < 0
      true
    end
    ...
    def version=(version)
      validator = EnumAttributeValidator.new('String', ["v2", "v3"])
    ...
    # @param [Object] min_score Value to be assigned
    def min_score=(min_score)
      if min_score.nil?
        fail ArgumentError, 'min_score cannot be nil'
      end

      if min_score > 1
        fail ArgumentError, 'invalid value for "min_score", must be smaller than or equal to 1.'
      end

      if min_score < 0
        fail ArgumentError, 'invalid value for "min_score", must be greater than or equal to 0.'
      end
    ...
    def hash
      [enabled, version, site_key, min_score].hash
    end
