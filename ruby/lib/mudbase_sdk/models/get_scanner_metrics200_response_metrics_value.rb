...
module MudbaseSDK
  class GetScannerMetrics200ResponseMetricsValue < ApiModelBase
    attr_accessor :last_scanned_block
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::GetScannerMetrics200ResponseMetricsValue` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::GetScannerMetrics200ResponseMetricsValue`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'last_scanned_block')
        self.last_scanned_block = attributes[:'last_scanned_block']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          last_scanned_block == o.last_scanned_block &&
    ...
    def hash
      [last_scanned_block, current_block, lag, last_updated].hash
    end
