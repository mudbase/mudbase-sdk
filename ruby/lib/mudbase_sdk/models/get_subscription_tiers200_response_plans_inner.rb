...
module MudbaseSDK
  class GetSubscriptionTiers200ResponsePlansInner < ApiModelBase
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::GetSubscriptionTiers200ResponsePlansInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::GetSubscriptionTiers200ResponsePlansInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'price_yearly')
        self.price_yearly = attributes[:'price_yearly']
      end
      ...
      if attributes.key?(:'overages')
        self.overages = attributes[:'overages']
      end
    ...
    def hash
      [id, name, description, price, price_yearly, currency, price_id, limits, overages, enforcement].hash
    end
