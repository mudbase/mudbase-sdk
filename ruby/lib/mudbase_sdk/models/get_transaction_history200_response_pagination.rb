...
module MudbaseSDK
  class GetTransactionHistory200ResponsePagination < ApiModelBase
    attr_accessor :page
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::GetTransactionHistory200ResponsePagination` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::GetTransactionHistory200ResponsePagination`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'has_previous_page')
        self.has_previous_page = attributes[:'has_previous_page']
      end
    end
    ...
    def hash
      [page, limit, count, total, total_pages, has_next_page, has_previous_page].hash
    end
