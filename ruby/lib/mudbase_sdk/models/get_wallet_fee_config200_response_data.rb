  ...
  class GetWalletFeeConfig200ResponseData < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::GetWalletFeeConfig200ResponseData` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::GetWalletFeeConfig200ResponseData`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'fee_percentage')
        self.fee_percentage = attributes[:'fee_percentage']
      end
    end
    ...
    def list_invalid_properties
      ...
      if !@fee_percentage.nil? && @fee_percentage > 1
        invalid_properties.push('invalid value for "fee_percentage", must be smaller than or equal to 1.')
      end

      if !@fee_percentage.nil? && @fee_percentage < 0
        invalid_properties.push('invalid value for "fee_percentage", must be greater than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if !@fee_percentage.nil? && @fee_percentage > 1
      return false if !@fee_percentage.nil? && @fee_percentage < 0
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] fee_percentage Value to be assigned
    def fee_percentage=(fee_percentage)
      if fee_percentage.nil?
        fail ArgumentError, 'fee_percentage cannot be nil'
      end

      if fee_percentage > 1
        fail ArgumentError, 'invalid value for "fee_percentage", must be smaller than or equal to 1.'
      end

      if fee_percentage < 0
        fail ArgumentError, 'invalid value for "fee_percentage", must be greater than or equal to 0.'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          enabled == o.enabled &&
          fee_percentage == o.fee_percentage
    end
