...
module MudbaseSDK
  class GetWalletPrivateKey200ResponseData < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::GetWalletPrivateKey200ResponseData` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::GetWalletPrivateKey200ResponseData`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'is_custom_key')
        self.is_custom_key = attributes[:'is_custom_key']
      end
    end
    ...
    def hash
      [wallet_id, currency, address, private_key, is_custom_key].hash
    end
