...
module MudbaseSDK
  class GetWebhookConfig200ResponseDataTransformationsInner < ApiModelBase
    attr_accessor :type
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::GetWebhookConfig200ResponseDataTransformationsInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::GetWebhookConfig200ResponseDataTransformationsInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["jsonPath", "template", "fieldMap", "filter", "flatten", "version"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["jsonPath", "template", "fieldMap", "filter", "flatten", "version"])
