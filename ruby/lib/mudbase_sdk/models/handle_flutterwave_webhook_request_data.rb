...
module MudbaseSDK
  class HandleFlutterwaveWebhookRequestData < ApiModelBase
    attr_accessor :id

    attr_accessor :tx_ref

    attr_accessor :flw_ref
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::HandleFlutterwaveWebhookRequestData` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::HandleFlutterwaveWebhookRequestData`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'flw_ref')
        self.flw_ref = attributes[:'flw_ref']
      end
    ...
    def hash
      [id, tx_ref, flw_ref, amount, currency, status, customer, meta].hash
    end
