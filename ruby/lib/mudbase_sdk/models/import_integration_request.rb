...
module MudbaseSDK
  class ImportIntegrationRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::ImportIntegrationRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::ImportIntegrationRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'integration_data')
        self.integration_data = attributes[:'integration_data']
      else
        self.integration_data = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @integration_data.nil?
        invalid_properties.push('invalid value for "integration_data", integration_data cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @integration_data.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] integration_data Value to be assigned
    def integration_data=(integration_data)
      if integration_data.nil?
        fail ArgumentError, 'integration_data cannot be nil'
      end
