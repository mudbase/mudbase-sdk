...
module MudbaseSDK
  class InitializeOrgPlanCheckout200ResponseData < ApiModelBase
    # Payment URL
    attr_accessor :link
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::InitializeOrgPlanCheckout200ResponseData` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::InitializeOrgPlanCheckout200ResponseData`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [link, tx_ref, provider_ref, billing_cycle, amount, amount_cents].hash
    end
