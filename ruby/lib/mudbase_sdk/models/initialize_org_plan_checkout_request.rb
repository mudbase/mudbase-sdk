...
module MudbaseSDK
  class InitializeOrgPlanCheckoutRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::InitializeOrgPlanCheckoutRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::InitializeOrgPlanCheckoutRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      plan_name_validator = EnumAttributeValidator.new('String', ["starter", "growth", "scale"])
      return false unless plan_name_validator.valid?(@plan_name)
    ...
    # @param [Object] plan_name Object to be assigned
    def plan_name=(plan_name)
      validator = EnumAttributeValidator.new('String', ["starter", "growth", "scale"])
      unless validator.valid?(plan_name)
        fail ArgumentError, "invalid value for \"plan_name\", must be one of #{validator.allowable_values}."
      end
      @plan_name = plan_name
    end
    ...
    def hash
      [plan_name, billing_cycle, redirect_url].hash
    end
