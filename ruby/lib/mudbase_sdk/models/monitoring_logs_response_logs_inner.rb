...
module MudbaseSDK
  class MonitoringLogsResponseLogsInner < ApiModelBase
    attr_accessor :id
    ...
    def self.openapi_types
      {
        ...
        :'user' => :'MonitoringLogsResponseLogsInnerUser',
        :'project' => :'Object',
        :'metadata' => :'Object'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::MonitoringLogsResponseLogsInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::MonitoringLogsResponseLogsInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'activity_title')
        self.activity_title = attributes[:'activity_title']
      end

      if attributes.key?(:'activity_detail')
        self.activity_detail = attributes[:'activity_detail']
      end
    ...
    def hash
      [id, timestamp, level, message, action, activity_title, activity_detail, user, project, metadata].hash
    end
