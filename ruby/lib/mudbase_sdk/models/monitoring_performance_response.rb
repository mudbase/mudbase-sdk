...
module MudbaseSDK
  class MonitoringPerformanceResponse < ApiModelBase
    attr_accessor :period

    attr_accessor :metrics

    attr_accessor :top_endpoints
    ...
    def self.openapi_types
      {
        :'period' => :'String',
        :'metrics' => :'MonitoringPerformanceResponseMetrics',
        :'top_endpoints' => :'Array<Object>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::MonitoringPerformanceResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::MonitoringPerformanceResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'top_endpoints')
        if (value = attributes[:'top_endpoints']).is_a?(Array)
          self.top_endpoints = value
        end
      end
    end
    ...
    def hash
      [period, metrics, top_endpoints].hash
    end
