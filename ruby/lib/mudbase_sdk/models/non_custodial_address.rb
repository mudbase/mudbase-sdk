...
module MudbaseSDK
  class NonCustodialAddress < ApiModelBase
    attr_accessor :_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::NonCustodialAddress` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::NonCustodialAddress`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      chain_validator = EnumAttributeValidator.new('String', ["ethereum", "binance", "polygon", "celo", "bitcoin"])
    ...
    def chain=(chain)
      validator = EnumAttributeValidator.new('String', ["ethereum", "binance", "polygon", "celo", "bitcoin"])
    ...
    def hash
      [_id, address, chain, org, project, derivation_path, label, is_active, registered_at, last_synced_at, created_at, updated_at].hash
    end
