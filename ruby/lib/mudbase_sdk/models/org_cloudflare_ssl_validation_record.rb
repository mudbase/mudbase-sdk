...
module MudbaseSDK
  class OrgCloudflareSslValidationRecord < ApiModelBase
    attr_accessor :txt_name

    attr_accessor :txt_value
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::OrgCloudflareSslValidationRecord` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::OrgCloudflareSslValidationRecord`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'txt_name')
        self.txt_name = attributes[:'txt_name']
      end

      if attributes.key?(:'txt_value')
        self.txt_value = attributes[:'txt_value']
      end
    ...
    def hash
      [txt_name, txt_value, http_url, http_body, cname, cname_target].hash
    end
