...
module MudbaseSDK
  class PatchProjectFcmConfigRequestOneOf < ApiModelBase
    # Firebase service account JSON (client_email, private_key, etc.)
    attr_accessor :service_account_json
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::PatchProjectFcmConfigRequestOneOf` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::PatchProjectFcmConfigRequestOneOf`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @service_account_json.nil?
        invalid_properties.push('invalid value for "service_account_json", service_account_json cannot be nil.')
      end
    ...
    # @param [Object] service_account_json Value to be assigned
    def service_account_json=(service_account_json)
      if service_account_json.nil?
        fail ArgumentError, 'service_account_json cannot be nil'
      end
