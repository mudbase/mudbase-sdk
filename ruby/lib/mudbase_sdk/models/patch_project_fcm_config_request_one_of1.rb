...
module MudbaseSDK
  class PatchProjectFcmConfigRequestOneOf1 < ApiModelBase
    attr_accessor :clear
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::PatchProjectFcmConfigRequestOneOf1` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::PatchProjectFcmConfigRequestOneOf1`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'clear')
    ...
    def list_invalid_properties
      ...
      if @clear.nil?
        invalid_properties.push('invalid value for "clear", clear cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @clear.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] clear Value to be assigned
    def clear=(clear)
      if clear.nil?
        fail ArgumentError, 'clear cannot be nil'
      end
