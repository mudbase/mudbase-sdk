...
module MudbaseSDK
  class ProjectSmsByoPatchRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::ProjectSmsByoPatchRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::ProjectSmsByoPatchRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'default_from')
        self.default_from = attributes[:'default_from']
      end
    ...
    def valid?
      ...
      provider_validator = EnumAttributeValidator.new('String', ["twilio", "termii", "africastalking", "none"])
    ...
    def provider=(provider)
      validator = EnumAttributeValidator.new('String', ["twilio", "termii", "africastalking", "none"])
    ...
    def hash
      [enabled, provider, default_from, config].hash
    end
