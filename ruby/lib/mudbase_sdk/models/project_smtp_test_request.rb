  ...
  class ProjectSmtpTestRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::ProjectSmtpTestRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::ProjectSmtpTestRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'use_saved')
        self.use_saved = attributes[:'use_saved']
      else
        self.use_saved = true
      end
    ...
    def hash
      [to, use_saved, host, port, secure, auth_user, auth_pass, from_email, from_name].hash
    end
