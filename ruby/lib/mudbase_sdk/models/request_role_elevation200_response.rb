...
module MudbaseSDK
  class RequestRoleElevation200Response < ApiModelBase
    ...
    attr_accessor :estimated_approval_time
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::RequestRoleElevation200Response` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::RequestRoleElevation200Response`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'next_steps')
        if (value = attributes[:'next_steps']).is_a?(Array)
          self.next_steps = value
        end
      end

      if attributes.key?(:'estimated_approval_time')
        self.estimated_approval_time = attributes[:'estimated_approval_time']
      end
    end
    ...
    def valid?
      ...
      workflow_validator = EnumAttributeValidator.new('String', ["auto_approved", "pending_admin_approval", "pending_requirements", "manual_approval"])
      return false unless workflow_validator.valid?(@workflow)
      status_validator = EnumAttributeValidator.new('String', ["approved", "pending"])
    ...
    def workflow=(workflow)
      validator = EnumAttributeValidator.new('String', ["auto_approved", "pending_admin_approval", "pending_requirements", "manual_approval"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["approved", "pending"])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          estimated_approval_time == o.estimated_approval_time
    end
    ...
    def hash
      [message, request_id, workflow, status, next_steps, estimated_approval_time].hash
    end
