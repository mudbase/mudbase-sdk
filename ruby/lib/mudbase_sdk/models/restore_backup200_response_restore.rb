...
module MudbaseSDK
  class RestoreBackup200ResponseRestore < ApiModelBase
    attr_accessor :_id
    ...
    attr_accessor :estimated_completion
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::RestoreBackup200ResponseRestore` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::RestoreBackup200ResponseRestore`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'estimated_completion')
        self.estimated_completion = attributes[:'estimated_completion']
      end
    end
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('String', ["in_progress", "completed", "failed"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["in_progress", "completed", "failed"])
    ...
    def hash
      [_id, status, restore_mode, started_at, estimated_completion].hash
    end
