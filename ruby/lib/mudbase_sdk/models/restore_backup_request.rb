...
module MudbaseSDK
  class RestoreBackupRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::RestoreBackupRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::RestoreBackupRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'restore_mode')
        self.restore_mode = attributes[:'restore_mode']
      end
    ...
    def valid?
      ...
      restore_mode_validator = EnumAttributeValidator.new('String', ["replace", "merge"])
      return false unless restore_mode_validator.valid?(@restore_mode)
      return false if @confirmation.nil?
      confirmation_validator = EnumAttributeValidator.new('String', ["RESTORE_DATA"])
      return false unless confirmation_validator.valid?(@confirmation)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] restore_mode Object to be assigned
    def restore_mode=(restore_mode)
      validator = EnumAttributeValidator.new('String', ["replace", "merge"])
      unless validator.valid?(restore_mode)
        fail ArgumentError, "invalid value for \"restore_mode\", must be one of #{validator.allowable_values}."
      end
    ...
    def confirmation=(confirmation)
      validator = EnumAttributeValidator.new('String', ["RESTORE_DATA"])
    ...
    def hash
      [restore_mode, collections, confirmation].hash
    end
