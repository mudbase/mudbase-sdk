  ...
  class SendMessageRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::SendMessageRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::SendMessageRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'e2ee')
        self.e2ee = attributes[:'e2ee']
      end
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["text", "image", "video", "audio", "file", "location", "contact"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["text", "image", "video", "audio", "file", "location", "contact"])
    ...
    def hash
      [type, content, e2ee, reply_to, mentions].hash
    end
