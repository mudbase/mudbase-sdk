...
module MudbaseSDK
  class UpdateCollectionPermissionsRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::UpdateCollectionPermissionsRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::UpdateCollectionPermissionsRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'data_scope')
        self.data_scope = attributes[:'data_scope']
      end

      if attributes.key?(:'owner_field')
        self.owner_field = attributes[:'owner_field']
      end
    end
    ...
    def valid?
      ...
      data_scope_validator = EnumAttributeValidator.new('String', ["all", "own"])
      return false unless data_scope_validator.valid?(@data_scope)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] data_scope Object to be assigned
    def data_scope=(data_scope)
      validator = EnumAttributeValidator.new('String', ["all", "own"])
      unless validator.valid?(data_scope)
        fail ArgumentError, "invalid value for \"data_scope\", must be one of #{validator.allowable_values}."
      end
    ...
    def hash
      [actions, conditions, data_scope, owner_field].hash
    end
