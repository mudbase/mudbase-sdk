...
module MudbaseSDK
  class UpdateFunctionRequestRetryPolicy < ApiModelBase
    attr_accessor :enabled

    attr_accessor :max_retries

    attr_accessor :backoff_ms
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::UpdateFunctionRequestRetryPolicy` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::UpdateFunctionRequestRetryPolicy`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'backoff_ms')
        self.backoff_ms = attributes[:'backoff_ms']
      end
    end
    ...
    def hash
      [enabled, max_retries, backoff_ms].hash
    end
