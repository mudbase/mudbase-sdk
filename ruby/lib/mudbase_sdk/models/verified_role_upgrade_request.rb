...
module MudbaseSDK
  class VerifiedRoleUpgradeRequest < ApiModelBase
    attr_accessor :target_role
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::VerifiedRoleUpgradeRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::VerifiedRoleUpgradeRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'target_role')
        self.target_role = attributes[:'target_role']
      else
        self.target_role = nil
      end
    ...
    def list_invalid_properties
      ...
      if @target_role.nil?
        invalid_properties.push('invalid value for "target_role", target_role cannot be nil.')
      end
    ...
    # @param [Object] target_role Value to be assigned
    def target_role=(target_role)
      if target_role.nil?
        fail ArgumentError, 'target_role cannot be nil'
      end
    ...
    def hash
      [target_role, payment_intent_id, verification_id].hash
    end
