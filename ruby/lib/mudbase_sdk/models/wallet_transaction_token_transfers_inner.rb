...
module MudbaseSDK
  class WalletTransactionTokenTransfersInner < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::WalletTransactionTokenTransfersInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::WalletTransactionTokenTransfersInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'is_incoming')
        self.is_incoming = attributes[:'is_incoming']
      end
    end
    ...
    def hash
      [token_address, from, to, value, formatted_amount, token_symbol, token_decimals, is_incoming].hash
    end
