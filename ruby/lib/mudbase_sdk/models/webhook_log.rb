  ...
  class WebhookLog < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MudbaseSDK::WebhookLog` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MudbaseSDK::WebhookLog`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('String', ["pending", "success", "failed", "retrying"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["pending", "success", "failed", "retrying"])
    ...
    def hash
      [_id, org, project, webhook_id, url, method, event, status, payload, headers, response, duration, attempts, max_attempts, error, next_retry, created_at, updated_at].hash
    end
