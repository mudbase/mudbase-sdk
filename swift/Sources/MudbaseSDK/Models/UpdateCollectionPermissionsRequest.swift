//
// UpdateCollectionPermissionsRequest.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation

public struct UpdateCollectionPermissionsRequest: Sendable, Codable, Hashable {

    public enum Actions: String, Sendable, Codable, CaseIterable {
        case create = "create"
        case read = "read"
        case update = "update"
        case delete = "delete"
    }
    public enum DataScope: String, Sendable, Codable, CaseIterable {
        case all = "all"
        case own = "own"
    }
    public var actions: [Actions]?
    public var conditions: JSONValue?
    /** `all` = no automatic row-owner filter. `own` = only documents where the owner field matches the authenticated app user. */
    public var dataScope: DataScope?
    /** Optional override for the document field when dataScope is `own` (default `settings.dataOwnerField`, usually `createdBy`). */
    public var ownerField: String?

    public init(actions: [Actions]? = nil, conditions: JSONValue? = nil, dataScope: DataScope? = nil, ownerField: String? = nil) {
        self.actions = actions
        self.conditions = conditions
        self.dataScope = dataScope
        self.ownerField = ownerField
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case actions
        case conditions
        case dataScope
        case ownerField
    }

    // Encodable protocol methods

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encodeIfPresent(actions, forKey: .actions)
        try container.encodeIfPresent(conditions, forKey: .conditions)
        try container.encodeIfPresent(dataScope, forKey: .dataScope)
        try container.encodeIfPresent(ownerField, forKey: .ownerField)
    }
}

